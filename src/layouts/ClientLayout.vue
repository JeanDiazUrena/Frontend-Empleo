...
const handleNotifClick = async (notif) => {
  ...
  try {
    const target = await resolveNotificationTarget(notif, 'client');
...
const isActive = (path) => {
  if (path === 'profile') return route.name === 'ClientProfile';
...
const goToProfile  = () => { isMenuOpen.value = false; router.push('/client/profile'); };
const goToSettings = () => { isMenuOpen.value = false; router.push('/client/settings'); };
...
/* NAVBAR */
.dash-navbar { height: 70px; background: white; border-bottom: 1px solid #e5e7eb; display: flex; align-items: center; justify-content: space-between; padding: 0 30px; position: fixed; top: 0; left: 0; right: 0; z-index: 1000; }
...
/* DROPDOWN FLOTANTE */
.dropdown-menu { position: absolute; top: 60px; right: 0; width: 220px; background: white; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); border: 1px solid #e2e8f0; overflow: hidden; z-index: 1100; }
